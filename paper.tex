...
\title[A Burgers--Fisher Equation for Prime Gap Convergence]{%
...
\begin{abstract}
We prove that the velocity field $u = -\partial_v \log f$, where $f$ is the probability density of normalized prime gaps $v = g/\log x$, satisfies a Burgers--Fisher equation in a porous medium:
\[
\partial_t u + 2v\,u\,\partial_v u = v\,\partial_v^2 u + (v{+}2)\,\partial_v u + u(1{-}u).
\]
The Fisher--KPP reaction term $u(1{-}u)$ emerges naturally from the Fokker--Planck structure and drives $u$ toward $1$, which corresponds to the exponential (Poisson) distribution.
The linearized perturbation operator has associated Laguerre polynomial eigenfunctions with spectral gap~$1$.
Under a decaying sieve forcing, perturbations vanish as $O(1/(\log x)^\beta)$.
...
Under the Hardy--Littlewood conjecture:
$f(v,s) \propto W(vs) \cdot e^{-v}$,
where $W(g) = \prod_{p|g,\,p>2} (p{-}1)/(p{-}2)$.
...
Empirical $\MI_3 = 0.302$ bits vs.\ theoretical $0.250$ bits; the $21\%$ excess is the Lemke Oliver--Soundararajan bias~\cite{LO2016}, whose Main Conjecture predicts decay as $O((\log\log x)/\log x)$.
...
The relationship between our work and Gallagher's is analogous to the relationship between the Ising model and the thermodynamic observation that magnets lose magnetization above the Curie temperature: the observation was known first; the model explains the mechanism.
