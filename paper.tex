...
The sequence of prime numbers is entirely deterministic, sculpted by the rigid constraints of the sieve of Eratosthenes.
Yet at macroscopic scales, the gaps between consecutive primes mimic a perfectly memoryless random process: Gallagher~\cite{Gallagher1976} proved conditionally on the Hardy--Littlewood conjecture~\cite{HL1923} that prime counts in short intervals converge to Poisson statistics, implying that normalized gaps approach the exponential distribution $f(v) = e^{-v}$.
How does a system built on strict divisibility rules dissolve into statistical randomness?

While the \emph{destination} (Poisson) is conditionally established, the \emph{dynamics} of this convergence have remained obscure.
Lemke Oliver and Soundararajan~\cite{LO2016} discovered that consecutive primes exhibit unexpected biases in their residue patterns---a signature of the sieve's lingering grip---decaying as $O((\log\log x)/\log x)$; Holt~\cite{Holt2024} modeled these biases via sieve dynamics.
But no differential equation governing the transition, and no quantitative convergence rate, have been established.

In this paper, we propose that the answer lies in the language of fluid mechanics.
Rather than treating prime gaps as a static probability distribution, we transform the gap density into a \emph{velocity field} and model the system as a physical flow through a porous medium---the sieve.
We show that if the gap density evolves under a Feller Fokker--Planck equation~\cite{CIR1985, Risken1989}---whose state-dependent diffusivity mirrors the arithmetic rigidity of small gaps---the resulting velocity field satisfies a nonlinear Burgers--Fisher equation~\cite{Fisher1937, KPP1937}.
The Poisson regime is not a statistical coincidence but the unique stable equilibrium of this PDE, enforced by a spectral gap of~$1$ and a self-correcting Fisher--KPP reaction term $u(1-u)$ that emerges naturally from the Fokker--Planck structure.
Numerical verification at 11 scales yields a decay rate of $\approx (\log x)^{-0.19}$, driven overwhelmingly by the prime $p = 3$.
...
Cohen~\cite{Cohen2024} independently confirmed that the moments of prime gaps converge to those of the
