...
We verify numerically that the mutual information between consecutive gaps decays as $(\log x)^{-0.19}$---the first quantitative estimate of this rate---and that prime $p = 3$ carries $94\%$ of the sieve signal.
\end{abstract}
...
Despite decades of work~\cite{LO2016, Kuperberg2023, Holt2024}, no quantitative convergence rate has been established.
...
This is the first quantitative estimate of the convergence rate to Poisson.

\subsection{Prime $3$ dominance}
$\MI_3 / \MI_{\mathrm{total}} \approx 94\%$ at all scales.
Empirical $\MI_3 = 0.302$ bits vs.\ theoretical $0.250$ bits; the $21\%$ excess is the Lemke Oliver--Soundararajan bias~\cite{LO2016}, decaying as $O((\log\log x)/\log x)$~\cite{Holt2024}.
The structural zeros $P(1,1) = P(2,2) = 0$ hold exactly at all scales.
...
\begin{enumerate}
\item \textbf{Derive} $\|\varphi(\cdot, s)\| = O(1/s^\beta)$ from the Hardy--Littlewood conjecture and equidistribution of primes in arithmetic progressions.
\item \textbf{Prove} MI $\to 0$ from the Burgers--Fisher framework (requires a joint version of Hypothesis~\ref{hyp:sieve}).
\item \textbf{Compute} the exponent $\alpha \approx 0.19$ from first principles: it should follow from the pore strength $\delta_3 = 1$ and the equidistribution rate of gaps mod~$3$.
\end{enumerate}
...
\bibitem{Kuperberg2023}
G.~Kuperberg, \emph{Sums of singular series with large sets}, Quart.\ J.\ Math.\ \textbf{74} (2023), 1489--1518.
